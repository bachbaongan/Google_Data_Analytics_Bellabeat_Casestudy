...
![](/Users/clarabach/Downloads/Data Analyst/Project/Bellabeat Project/Bellabeat Icon.png)
...
#### The Credibility & Integrity of the data:
The data follow the ***ROCCC Analysis***: 
...
```{r importdata, message=FALSE, warning=TRUE}
setwd("/Users/clarabach/Downloads/Data Analyst/Project/Bellabeat Project")
```
...
``` 
```{r ss}
write_csv(hourly_step,"hourly_step.csv")
```
...
```{r save}
write_csv(merge_asw, "merge_asw.csv")
write_csv(merge_aswh, "merge_aswh.csv")
```
...
```{r aclevdis, echo=FALSE}
distri <- merge_asw1 %>% 
  group_by(Activity_level) %>%
  summarize(sumlevel=  sum(minutes))%>%
  mutate(per = paste0(round(100*(sumlevel/sum(sumlevel)),2),"%")) 
  
ggplot(data=distri, aes(x="", y=sumlevel, fill=Activity_level)) +
  geom_col() +
  geom_text(aes(label = per),position = position_stack(vjust = 0.4),color="#543005") +
  coord_polar(theta="y") + theme_void()+ scale_fill_brewer(palette ="Accent")+
  labs(title="Activity level distribution classified by minutes")
```
